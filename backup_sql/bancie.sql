...
CREATE TABLE `SAMSUNG_PHONE_SCREEN` (
  ...
  KEY `DAY_ID` (`DAY_ID`),
  CONSTRAINT `SAMSUNG_PHONE_SCREEN_ibfk_1`
    FOREIGN KEY (`USER_ID`)
    REFERENCES `USERS` (`USER_ID`)
    ON DELETE CASCADE,
  CONSTRAINT `SAMSUNG_PHONE_SCREEN_ibfk_2`
    FOREIGN KEY (`DAY_ID`)
    REFERENCES `DAY` (`DAY_ID`)
    ON DELETE CASCADE
) ENGINE=InnoDB
...
CREATE TABLE `ACTIVITY_LOG` (
  ...
  `SOUND_BACKGROUND` SET(
    'category_music',
    'category_white_noise',
    'category_ambient_noise',
    'category_silence',
    'category_podcast',
  ...
  CONSTRAINT `ACTIVITY_LOG_ibfk_2`
    ...
    ON DELETE CASCADE,
  CONSTRAINT `ACTIVITY_LOG_ibfk_3`
    FOREIGN KEY (`DAY_ID`)
    REFERENCES `DAY` (`DAY_ID`)
    ON DELETE CASCADE
) ENGINE=InnoDB
